    ...
    with gr.Row():
        with gr.Column():
            with gr.Box():
                ...
                with gr.Row():
                    with gr.Column():
                        shared.gradio['temperature'] = gr.Slider(0.0006, 1.99, value=generate_params['temperature'], step=0.0001, label='temperature')
...
#      MODEL.coherence_json = coherence_json
      shared.settings['attention_type'] = attention_type
      shared.settings['coherence_json'] = coherence_json
    ...
    with gr.Blocks(css=css, analytics_enabled=False, title=title, theme=ui.theme) as shared.gradio['interface']:
        ...
        if shared.is_chat():
            ...
            with gr.Tab('Text generation', elem_id='main'):
                ...
                shared.gradio['textbox'] = gr.Textbox(label='Input', value='generate psytrance poetry')
                with gr.Row():
            ...
            with gr.Tab('Chat settings', elem_id='chat-settings'):
                ...
                with gr.Tab("Coherence"):
                  with gr.Row():
                    shared.gradio['attention_type'] = gr.Radio(["default", "random", "coherence"], label="Attention Type", value="default", interactive=True)#coherence
                    shared.gradio['coherence_json'] = gr.Text(label="Coherence JSON", value="{}", interactive=True)
                    shared.gradio['update_coherence_json'] = gr.Button("Update Coherence JSON")
                    shared.gradio['update_coherence_json'].click(update_coherence_json_click, inputs=[shared.gradio['attention_type'], shared.gradio['coherence_json']], queue=False)
